...
export interface RadialScrollGalleryProps
   ...
   * Receives the currently `activeIndex` (hovered or centered), the `hoveredIndex`, and the scroll-calculated `centeredIndex`.
   */
  children: (
  ...
  /** Active index for manual control */
  activeIndex?: number;
}
...
>(
  (
    ...
    const activeIndexVal = hoveredIndex !== null ? hoveredIndex : (activeIndex !== undefined ? activeIndex : centeredIndex);

    const childrenNodes = useMemo(
      () => React.Children.toArray(children(activeIndexVal, hoveredIndex, centeredIndex)),
      [children, activeIndexVal, hoveredIndex, centeredIndex]
    );
    const childrenCount = childrenNodes.length;

    useEffect(() => {
      if (childrenCount > 0 && centeredIndex === null) {
        // Initial calculation for rotation 0
        const topPos = 0; 
        const index = Math.round((topPos / 360) * childrenCount) % childrenCount;
        lastCenteredIndexRef.current = index;
        setCenteredIndex(index);
      }
    }, [childrenCount, centeredIndex]);
    ...
    useGSAP(
      () => {
        ...
        if (!prefersReducedMotion) {
          // If we have manual rotation, don't use the scroll-driven rotation hook for the container
            ...
            gsap.to(containerRef.current, {
              ...
              scrollTrigger: {
                ...
                onUpdate: (self) => {
                  ...
                  const topPos = (360 - normalizedRotation) % 360;
                  const index = Math.round((topPos / 360) * childrenCount) % childrenCount;
                  
                  if (lastCenteredIndexRef.current !== index) {
                    lastCenteredIndexRef.current = index;
                    setCenteredIndex(index);
                  }
                }
                  ...
                  */}
                  <div
                    ...
                    className={`
                      ...
                      ${activeIndexVal === index ? 'scale-125 -translate-y-8' : 'scale-100'}
                      ${
                        activeIndexVal !== null && activeIndexVal !== index
